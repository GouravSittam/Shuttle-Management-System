...
export const CometDial: React.FC<CometDialProps> = ({
  value,
  defaultValue = 62,
  min = 0,
  max = 100,
  ...
  return (
    <div
      className={`group relative select-none [-webkit-touch-callout:none] [-webkit-tap-highlight-color:transparent] [width:var(--cd-size)] [height:var(--cd-size)] [color:var(--cd-ink)] data-[disabled]:opacity-55 comet-dial-root${className ? ` ${className}` : ''}`}
    ...
    >
      <svg
        ref={svgRef}
        className="comet-dial-svg absolute inset-0 block h-full w-full cursor-grab touch-none rounded-full outline-none group-data-[dragging]:cursor-grabbing group-data-[disabled]:pointer-events-none group-data-[disabled]:cursor-default"
        viewBox="0 0 200 200"
        role="slider"
      ...
      >
        <path
          className="comet-dial-track fill-none opacity-[0.24] [stroke:var(--cd-ink)] [stroke-linecap:round]"
          d={arcPath(start, end)}
          strokeWidth={thickness}
        />
        <path
          ref={litRef}
          className="comet-dial-lit fill-none opacity-70 [stroke:var(--cd-accent)] [stroke-linecap:round]"
          strokeWidth={thickness}
        />
        <g className="comet-dial-trails [&_path]:fill-none [&_path]:[stroke:var(--cd-accent)] [&_path]:[stroke-linecap:round]">
        ...
        <circle ref={headRef} className="comet-dial-head [fill:var(--cd-accent)]" r={thickness * 1.8} />
      </svg>
      <div className="comet-dial-readout-wrap pointer-events-none absolute inset-0 grid place-items-center" aria-hidden="true">
        <span className="comet-dial-readout inline-flex items-baseline gap-[0.06em] leading-none font-medium tracking-[-0.04em] tabular-nums [font-size:var(--cd-figure)]">
          <span ref={figure} className="comet-dial__figure" />
          {unit ? <span className="comet-dial-unit text-[0.5em] opacity-50">{unit}</span> : null}
        </span>
      </div>
    </div>
  );
