...
export function Peel({
  ...
  return (
    <div
      className={`peel-wrapper ${!native ? "peel-fallback-active" : ""} ${className}`}
      data-side={options.side || "left"}
      style={{ position: "relative", ...style }}
    >
      {/* Dogear hint on peeling edge */}
      <div className="peel-dogear-hint" title="Peel to inspect security layer" />

      {/* Under layer (revealed when peeled) */}
      {under ? (
        <div
          ref={underRef}
          className="peel-under-layer"
          style={
            native
              ? {
                  position: "absolute",
      ...
      {/* Fallback container for standard browsers */}
      {!native ? (
        <div
          ref={contentRef}
          className="peel-fallback-container"
          style={{
            position: "relative",
            width: "100%",
            height: "100%",
